...
class InstaSupabaseUser extends BaseAuthUser {
  InstaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> instaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = InstaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
